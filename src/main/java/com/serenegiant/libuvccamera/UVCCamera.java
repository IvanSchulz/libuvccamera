...
//    		mNativePtr = 0;	// nativeDestroy is called, so don't clear here
	...
	public void setPreviewSize(final int width, final int height) {
		if (!isLoaded) return;
		setPreviewSize(width, height, DEFAULT_CAMERA_ANGLE, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, mCurrentFrameFormat);
	}
	...
	public void setPreviewSize(final int width, final int height, final int frameFormat) {
		if (!isLoaded) return;
		setPreviewSize(width, height, DEFAULT_CAMERA_ANGLE, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, frameFormat);
	}
	...
	public void setPreviewSize(final int width, final int height, final int cameraAngle, final int frameFormat) {
		if (!isLoaded) return;
		setPreviewSize(width, height, cameraAngle, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, frameFormat);
	}
